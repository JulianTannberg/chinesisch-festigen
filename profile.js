(function(){
  ...
  function renderCompact(host){
    ...
    host.querySelector("[data-profile-reset]").addEventListener("click", async () => {
      const id = new URLSearchParams(location.search).get("id");
      const ok = (typeof cfConfirm === "function")
        ? await cfConfirm("Alle Prozente und Statistiken dieses Kapitels werden gelöscht. Dein Name bleibt erhalten.", {title:"Fortschritt zurücksetzen?"})
        : window.confirm("Möchtest du den Fortschritt dieses Kapitels wirklich zurücksetzen?");
      if(!ok) return;
